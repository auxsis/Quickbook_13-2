...
class SaleOrder(models.Model):
    ...
    def action_invoice_create(self, grouped=False, final=False):
        ...
        if self.patho:
            if len(self.patho_testreq_id.invoice_ids)== 0:
                self.patho_testreq_id.invoice_ids = [(6, 0, res)]
            else:
                self.patho_testreq_id.invoice_ids = [(4, res[0])]
            self.patho_testreq_id.invoice_count = len(set(self.invoice_ids.ids))
        return res

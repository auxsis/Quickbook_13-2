...
class ir_attachment(models.Model):
    ...
    To-do:
     * in direct sync: do not sync files with stated folder_id in standard sync
     * in driect sync: move a file from sync.object to documents.folder. From folder to a folder if changed
     * in driect sync: do not reconcile stand-alone attachments with folder_id
     * in backward sync: do not delete a file if it has a parent folder in standard sync
     * for cloud files do not apply onchange for url
    """
    ...
    def _on_url_change(self):
        """
        Override not to break url from Cloud Clients
        """
        pass

    previous_folder_id = fields.Many2one("documents.folder", string="Previous Folder")
    ...
    def _sync_documents_folders(self):
        ...
        parent_folder_ids = self.env["documents.folder"].search([("parent_folder_id", "=", False)])
    ...
    def _find_or_create_documents_dir_directory(self):
        ...
        if not key:
            key, path  = self._create_folder(folder_name="Odoo Docs", parent_folder_key=root_key, parent_folder_path=root_path)

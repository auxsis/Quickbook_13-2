...
class documents_folder(models.Model):
    ...
    To-do
     * open folder from folder views (mainly from new interface)
    """
    ...
    def _make_recursive_update(self, parent_key, parent_path):
        ...
        for child_folder in self.children_folder_ids:
            child_folder._make_recursive_update(parent_key=parent_key, parent_path=parent_path)
    ...
    def _sync_folder_attachments(self):
        ...
        for attach in excluded_attachment_ids:
            ...
            if s_folder_id:
                attach._move_attachment(folder_id=s_object_id)

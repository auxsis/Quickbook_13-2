...
class shopify_product_template_ept(models.Model):
    ...
    def get_total_sync_variants(self):
        ...
        for template in self:
            variants = shopify_product_obj.search(
                [('id', 'in', template.shopify_product_ids.ids), ('exported_in_shopify', '=', True),
                 ('variant_id', '!=', False)])
    ...
    fulfillment_service = fields.Selection([('manual', 'Manual'), ('shopify', 'shopify'), ('gift_card', 'Gift Card')],
                                           default='manual')
    ...
    shopify_gallery_image_ids = fields.One2many('shopify.product.image.ept', 'shopify_product_tmpl_id',
                                                'Gallery Images')
    ...
    def set_variant_sku(self, result, product_template, instance):
        ...
        for variation in result.get('variants'):
            ...
            if not (sku or barcode) and odoo_product:
                new_barcode = instance.shopify_instance_prefix + str(variation.get("id"))
                odoo_product.barcode = new_barcode
    ...
    def is_product_importable(self, result, instance, odoo_product, shopify_product):
        ...
        if not odoo_product and not shopify_product:
            for variantion in variants:
                ...
                product_sku_barcodes.append({"name": template_title, "sku": sku or '', "barcode": barcode or ''})
                if barcode:
        ...
        if odoo_product:
            ...
            if not (product_count == 1 and odoo_template.product_variant_count == 1):
                if product_count == odoo_template.product_variant_count:
                    for shopify_prdct, odoo_prdct in zip(result.get('variants'), odoo_template.product_variant_ids):
                        ...
                        odoo_prdct and odoo_prdct.default_code and odoo_skus.append(odoo_prdct.default_code)
                        odoo_prdct and odoo_prdct.barcode and odoo_barcodes.append(odoo_prdct.barcode)
...
#                             return importable, message
                    ...
                    for sku in shopify_skus:
                        if sku not in odoo_skus:
                            message = "SKU not found in Odoo for Product: %s and SKU: %s." % (template_title, sku)
                            importable = False
                            return importable, message
        ...
        if shopify_product:
            ...
            for variantion in variants:
                ...
                barcode and shopify_barcodes.append(barcode)
                product_sku_barcodes.append({"name": template_title, "sku": sku or '', "barcode": barcode or ''})
                product_sku_variants.append({"variant_id": variant_id, "name": template_title, "sku": sku or '', "barcode": barcode or ''})
            ...
            if shopify_product and len(product_sku_barcodes) != shopify_product.shopify_template_id.total_variants_in_shopify:
                for product_sku_barcode in product_sku_barcodes:
                    odoo_product = odoo_product_obj.search([("default_code", "=", product_sku_barcode.get("sku"))], limit=1) or False
                    if odoo_product:
            ...
            if shopify_product.product_id.product_tmpl_id.product_variant_count != len(product_sku_barcodes): 
                ...
                for product_sku_variant in product_sku_variants:           
                    shopify_product = shopify_product_obj.search([("variant_id", "=", product_sku_variant.get("variant_id"))], limit=1) or False 
                    if shopify_product:                   
...
#                 return importable, message
        ...
        for image in image_response:
            ...
            if image.get('src'):
                ...
                shopify_variants = shopify_product_obj.search(
                    [('shopify_instance_id', '=', instance.id), ('variant_id', 'in', variant_ids)])
                ...
                if not instance.is_image_url:
                    ...
                    if shopify_gallery_image:
                        ...
                        shopify_gallery_image = shopify_product_img.search([('shopify_product_tmpl_id', '=', shopify_template.id), ('position', '=', image.get('position'))], limit=1)
                        if shopify_gallery_image:
                            shopify_gallery_image.write({'position': image.get('position'), 'height': image.get('height'), 'width': image.get('width'), 'image_id': img, 'shopify_variant_ids': shopify_product_ids and shopify_product_ids})
                        else:
                            shopify_product_img.create(
                            {'shopify_product_tmpl_id': shopify_template.id, 'shopify_instance_id': instance.id,
                            'image_id': img, 'shopify_image_id': image.get('id'), 'position': image.get('position'),
    ...
    @api.multi
    def sync_products(self, instance, sync_images_with_product=False, shopify_tmpl_id=False, update_price=False,
        ...
        for response_template in results:
            ...
            shopify_template = self.search(
                [('shopify_tmpl_id', '=', shopify_tmpl_id), ('shopify_instance_id', '=', instance.id)])
            ...
            for variant in response_template.get('variants'):
                ...
                if not instance.shopify_instance_prefix:
                    raise Warning("No prefix found for the Shopify instance : %s.\n"
                                  "Please set prefix under Configuration >> Settings >> "
                                  "Instance Prefix." % instance.name)
                new_barcode = instance.shopify_instance_prefix + str(variant_id)
                inventory_item_id = variant.get('inventory_item_id')
                shopify_variant = shopify_product_obj.search(
                    [('variant_id', '=', variant_id), ('shopify_instance_id', '=', instance.id)], limit=1)
                ...
                if not odoo_product and not shopify_variant and sku:
                    ...
                    if not shopify_variant and sku:
                        odoo_product = odoo_product_obj.search([('default_code', '=', sku)], limit=1)
                if not (odoo_product or shopify_variant or sku or barcode):
                    odoo_product = odoo_product_obj.search([('barcode', '=', new_barcode)], limit=1)
                product_category = product_category_obj.search([('name', '=', product_type),
                                                        ('is_shopify_product_cat', '=', True)])
                if not product_category:
                    product_category = product_category_obj.create({'name':product_type, 'is_shopify_product_cat':True})

                is_importable = True
                message = ""
                if not is_importable_checked:
                    is_importable, message = self.is_product_importable(response_template, instance, odoo_product,
                                                                        shopify_variant)
                    ...
                    if instance.auto_import_product:
                        if not onetime_call:
                            if len(response_template.get('variants')) > 1:
                                self.create_variant_product(response_template, instance, price, product_category)
                            else:
                                if sku or barcode:
                                    product_template_vals = {'name': template_title,
                                                             'default_code': sku or '',
                                                             'barcode': barcode or '',
                                                             'type': 'product',
                                                             'categ_id':product_category.id
                                ...
                                new_templ_obj = product_template_obj.create(product_template_vals)
                                if instance.import_price:
                                    if instance.pricelist_id.currency_id.id == new_templ_obj.company_id.currency_id.id:
                                        new_templ_obj.write({'list_price': price.replace(",", ".")})
                                    else:
                                        instance_currency = instance.pricelist_id.currency_id
...
#                                     continue
                            ...
                            onetime_call = True
                            if not odoo_product:
                                message = "Attribute(s) are not set properly in Product: %s." % (template_title)
                                transaction_log_obj.create(
                                    {'message': message,
                                     'mismatch_details': True,
                                     'type': 'product',
                        ...
                        message = "%s Product Not found for barcode %s and sku %s" % (template_title, barcode, sku)
                        log = transaction_log_obj.search(
                            [('shopify_instance_id', '=', instance.id), ('message', '=', message)])
                        if not log:
                            transaction_log_obj.create(
                ...
                if not shopify_variant:
                    ...
                    if not shopify_template:
                        ...
                        shopify_template = self.search([('product_tmpl_id', '=', odoo_product.product_tmpl_id.id),
                                                        ('shopify_instance_id', '=', instance.id)])
                        vals.update(
                            {'product_tmpl_id': odoo_product.product_tmpl_id.id, 'shopify_instance_id': instance.id,
                             'name': template_title,
                             'shopify_tmpl_id': shopify_tmpl_id, 'fulfillment_service': fulfillment_service,
                             'taxable': taxable,
                    ...
                    if not updated_template:
                        vals = {}
                        vals.update({'name': template_title, 'fulfillment_service': fulfillment_service,
                                     'taxable': taxable, 'created_at': created_at, 'updated_at': updated_at,
...
#             location_ids.append(instance.warehouse_id.lot_stock_id.id)
                    ...
                    for variant in template.shopify_product_ids:
                        if variant.variant_id and variant.product_id.type == 'product':
                            if not variant.inventory_item_id:
                                message = "Inventory Item Id not found for variant %s with id %s for instance %s while Update stock" % (variant.name, variant.variant_id, instance.name)
                                log = transaction_log_obj.search(
                                    [('shopify_instance_id', '=', instance.id), ('message', '=', message)])
                                if not log:
                                    transaction_log_obj.create(
                            ...
                            quantity = product_obj.get_stock_ept(variant.product_id, location_id.warehouse_id.id, variant.fix_stock_type, variant.fix_stock_value, instance.stock_field.name)
                            try:
                                shopify.InventoryLevel.set(location_id.shopify_location_id, variant.inventory_item_id,
                                                           int(quantity))
                            except Exception as e:                                
                                if e.response.code == 429 and e.response.msg == "Too Many Requests":
                                    time.sleep(5)
                                    ...
                                    message = "Error while Update stock for variant %s with id %s for instance %s\nError: %s" % (variant.name, variant.variant_id, instance.name, str(e.response.code) + " " + e.response.msg)
                                    log = transaction_log_obj.search(
                                        [('shopify_instance_id', '=', instance.id), ('message', '=', message)])
                                    if not log:
                                        transaction_log_obj.create(
    ...
    def update_products_in_shopify(self, instance, templates):
        ...
        for template in templates:
            ...
            for variant in template.shopify_product_ids:
                ...
                att_values = attribute_value_obj.search([('id', 'in', variant.product_id.attribute_value_ids.ids)],
                                                        order="attribute_id")
    ...
    def update_product_images(self, instance, shopify_template=False):
        ...
        for image in shopify_template.shopify_gallery_image_ids:
            ...
            shopify_image.variant_ids = [int(variant_id.variant_id) for variant_id in image.shopify_variant_ids]
    ...
    @api.model
    def export_products_in_shopify(self, instance, templates, update_price, update_stock, publish, update_image):
        instance.connect_in_shopify()
        ...
        for template in templates:
            ...
            for variant in template.shopify_product_ids:
                ...
                if update_stock:
                    quantity = product_obj.get_stock_ept(variant.product_id, instance.warehouse_id.id, variant.fix_stock_type, variant.fix_stock_value, instance.stock_field.name)
        ...
        if templates:
            ...
            for location_id in location_ids:
                ...
                inventory_id = self.env['stock.inventory'].create({
                    'name': 'Inventory For Instance %s And Shopify Location %s' % ((instance.name) + ' ' + datetime.now().strftime('%d-%m-%Y'), location_id.name),
                ...
                shopify_template_ids = shopify_template_ids and ','.join(shopify_template_ids) or False
                ...
                for inventory_level in inventory_levels:
                    ...
                    if shopify_product:
                        stock_inventory_line_obj.create(
                            {"inventory_id": inventory_id.id, "product_id": shopify_product.product_id.id,
...
class shopify_product_product_ept(models.Model):
    ...
    check_product_stock = fields.Selection(
        ...
        default='parent_product', help='If true than customers are allowed to place an order for the product variant when it is out of stock.')
    inventory_management = fields.Selection(
        ...
        default='parent_product', help="If you select 'Shopify tracks this product Inventory' than shopify tracks this product inventory.if select 'Dont track Inventory' then after we can not update product stock from odoo")

...
from odoo.addons.fedex_shipping_ept.fedex.services.address_validation_service import FedexAddressValidationRequest
import logging
...
class DeliveryCarrier(models.Model):
    ...
    fedex_droppoff_type = fields.Selection([('BUSINESS_SERVICE_CENTER', 'Business Service Center'),
                                           ...
                                           default='REGULAR_PICKUP', help="Identifies the method by which the package is to be tendered to FedEx.")
    ...
    fedex_weight_uom = fields.Selection([('LB', 'LB'),
                                         ('KG', 'KG')], default='LB', string="Weight UoM", help="Weight UoM of the Shipment")
    fedex_shipping_label_stock_type = fields.Selection([
                                               ...
                                               ('STOCK_4X9_TRAILING_DOC_TAB', 'Stock 4X9 Trailing Doc Tab')], string="Label Stock Type", help="Specifies the type of paper (stock) on which a document will be printed.")
    ...
    def prepare_shipment_request(self, instance, request_obj, shipper, recipient, package_type):
        ...
        request_obj.RequestedShipment.Shipper.Address.StreetLines = shipper.street and shipper.street2 and [shipper.street, shipper.street2] or [shipper.street]
        ...
        request_obj.RequestedShipment.Recipient.Address.StreetLines = recipient.street and recipient.street2 and [recipient.street, recipient.street2] or [recipient.street]
    ...
    @api.model
    def add_fedex_package(self, picking, ship_request, weight, package_count, number=1, master_tracking_id=False,package=False):
        ...
        if package and self.fedex_service_type in ['INTERNATIONAL_ECONOMY', 'INTERNATIONAL_FIRST', 'INTERNATIONAL_PRIORITY'] or (picking.partner_id.country_id.code == 'IN' and picking.picking_type_id.warehouse_id.partner_id.country_id.code == 'IN'):
            ...
            for operation in package.quant_ids:
    ...
    def fedex_ept_rate_shipment(self, orders):
        ...
        for order in orders :
            ...
            total_weight = self.convert_weight(order.company_id and order.company_id.weight_unit_of_measurement_id, self.weight_uom_id, total_weight)
            max_weight = self.convert_weight(order.company_id and order.company_id.weight_unit_of_measurement_id,
            ...
            try :
                ...
                rate_request = self.prepare_shipment_request(shipping_instance_id, rate_request, shipper_address, recipient_address, package_type)
...
#                 rate_request.RequestedShipment.PackageCount = total_package
                ...
                return {'success': False, 'price': 0.0, 'error_message': ERROR.value, 'warning_message': False}
                # raise ValidationError(ERROR.value)
            except FedexFailure as ERROR:
                return {'success': False, 'price': 0.0, 'error_message': ERROR.value, 'warning_message': False}
                # raise ValidationError(ERROR.value)
            ...
            for shipping_service in rate_request.response.RateReplyDetails:
                for rate_info in shipping_service.RatedShipmentDetails :
                    ...
                    if order.currency_id.name != rate_info.ShipmentRateDetail.TotalNetFedExCharge.Currency :
                        rate_currency = self.env['res.currency'].search([('name', '=', shipping_charge_currency)], limit=1)
                        if rate_currency :
    ...
    def fedex_ept_send_shipping(self, pickings):
        ...
        for picking in pickings :
            ...
            try :
                ...
                package_type = picking.package_ids and picking.package_ids[0].packaging_id.shipper_package_code or self.fedex_default_product_packaging_id.shipper_package_code
                ship_request = self.prepare_shipment_request(shipping_instance_id, ship_request, shipper_address, recipient_address, package_type)
...
#                     ship_request.RequestedShipment.ShippingDocumentSpecification.CertificateOfOrigin.DocumentFormat.StockType = ['PAPER_LETTER']
                ...
                for sequence, package in enumerate(picking.package_ids, start=1):
                    ...
                    package_weight = self.convert_weight(picking.weight_uom_id,self.weight_uom_id, package.shipping_weight)
                    ...
                    shipping_charge, tracking_number, label_binary_data = self.get_fedex_tracking_and_label(ship_request)
                    attachments.append(('Fedex-%s.%s' % (tracking_number, self.fedex_shipping_label_file_type), label_binary_data))
                ...
                if total_bulk_weight :
                    #Hemangi================================================================================
                    if self.fedex_service_type in ['INTERNATIONAL_ECONOMY', 'INTERNATIONAL_FIRST', 'INTERNATIONAL_PRIORITY'] or (picking.partner_id.country_id.code == 'IN' and picking.picking_type_id.warehouse_id.partner_id.country_id.code == 'IN'):
                        order = picking.sale_id
                        company = order.company_id or picking.company_id or self.env.user.company_id
                        order_currency = picking.sale_id.currency_id or picking.company_id.currency_id
                        commodity_country_of_manufacture = picking.picking_type_id.warehouse_id.partner_id.country_id.code
                        ...
                        for operation in picking.move_line_ids:
                            ...
                            Commodity.CountryOfManufacture = commodity_country_of_manufacture
                            Commodity.Weight.Units = commodity_weight_units
                            Commodity.Weight.Value = self.convert_weight(picking.weight_uom_id,self.weight_uom_id,operation.product_id.weight * operation.qty_done)
                            Commodity.Description = operation.product_id.name
                        ...
                        ship_request.RequestedShipment.CustomsClearanceDetail.CustomsValue.Currency = picking.sale_id.currency_id.name or picking.company_id.currency_id.name
                        #Hemangi===========================================================================================
                    ship_request = self.add_fedex_package(picking,ship_request, total_bulk_weight, package_count, number=package_count, master_tracking_id=fedex_master_tracking_id,package=False)
                    if self.fedex_onerate:
                        ship_request.RequestedShipment.SpecialServicesRequested.SpecialServiceTypes = ['FEDEX_ONE_RATE']
                    ship_request.send_request()
                    shipping_charge, tracking_number, label_binary_data = self.get_fedex_tracking_and_label(ship_request)
    ...
    def fedex_ept_cancel_shipment(self, picking):
        try :
            ...
            delete_request.TrackingId.TrackingNumber = picking.carrier_tracking_ref.split(',')[0] #master tracking number
            ...
            assert delete_request.response.HighestSeverity in ['SUCCESS', 'WARNING'], \
                "%s : %s" %(picking.carrier_tracking_ref.split(',')[0], delete_request.response.Notifications[0].Message)
        except FedexError as ERROR:
            raise ValidationError(ERROR.value)
        except FedexFailure as ERROR:
            raise ValidationError(ERROR.value)

...
class DeliveryCarrier(models.Model):
    ...
    def ups_get_shipping_rate(self, shipper_address, recipient_address, total_weight, picking_bulk_weight,
        ...
        print('declared_value',declared_value)     
        # built request data
...
#             etree.SubElement(shipment_weight, "Weight").text = "%s" % (total_weight)
        ...
        #hemangi - For Negotiation rate
        if self.use_ups_negotiation_rate:
            RateInformation = etree.SubElement(shipment,"RateInformation")
            etree.SubElement(RateInformation,"NegotiatedRatesIndicator")
        ...
        if service_code == self.ups_service_type:
            if shipment_charge:
                res.update({'ShippingCharge':negotiated_rates or shipment_charge or 0.0,
                            ...
                            'arrival_date':arrival_date.date() if arrival_date else None})
            else:
    ...
    def ups_ept_send_shipping(self, pickings):
        ...
        for picking in pickings:
            ...
            etree.SubElement(shipment_node, "Description").text = str(picking.note or picking.name or "")
            ...
            if picking_company_id.phone:
                etree.SubElement(shipper_node, "PhoneNumber").text = str(picking_company_id.phone.replace(' ',''))
            else:
            ...
            if picking_partner_id.phone:
                etree.SubElement(to_shipper_node, "PhoneNumber").text = str(picking_partner_id.phone.replace(' ',''))
            else:
            ...
            if self.ups_service_nature == "international":
                ...
                etree.SubElement(sold_to_node, "PhoneNumber").text = str(sold_to_partner_id.phone.replace(' ',''))
            ...
            if self.ups_service_nature == "international":
                ...
                for product in picking.move_lines:
                    ...
                    etree.SubElement(product_node, "CommodityCode").text = "%s" % (product.product_id.default_code)
